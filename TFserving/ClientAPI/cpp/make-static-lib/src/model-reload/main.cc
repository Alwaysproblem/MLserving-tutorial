...
typedef google::protobuf::RepeatedPtrField<tensorflow::serving::ModelConfig>
    RepeatModelConfig;
/*
...
int main(int argc, char** argv) {
  ...
  desc.add_options()
      ...
      ("help,h", "print usage message")(
          ...
          "model_version,v",
          value<int>(&model_version)->default_value(model_version),
          "the model version for prediction")(
          "model_version_label,l",
          value(&model_version_label)->default_value(model_version_label),
  ...
  if (status.ok()) {
    ...
    if (0 == response.status().error_code()) {
      std::cout << "model " << model_name << " reloaded successfully."
                << std::endl;
    } else {
